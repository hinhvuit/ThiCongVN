...
using AttcMN.Framework.DataEncryption;
using AttcMN.Framework.Exceptions;
using AttcMN.Framework.JwtBearer;
using AttcMN.Framework.Logging;
using System.Security.Claims;

namespace AttcMN.Common.Utils
{
    public static class SecurityUtils
    {
        ...
        public static LoginUser GetLoginUser(HttpRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                try
                {
                    ...
                    // 解析对应的权限以及用户信息
                    string uuid = claims.Where(c => c.Type.Equals(AttcMN.Data.Constants.LOGIN_USER_KEY)).First().Value;
        ...
        public static string GetToken(HttpRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(token) && token.StartsWith(AttcMN.Data.Constants.TOKEN_PREFIX))
            {
                token = token.Replace(AttcMN.Data.Constants.TOKEN_PREFIX, "");
            }
            return token;
        }
