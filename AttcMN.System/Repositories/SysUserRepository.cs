    ...
    public class SysUserRepository : BaseRepository<SysUser, SysUserDto>
    {
        ...
        public async Task<SysUserDto> GetUserDtoAsync(SysUserDto dto)
        {
            ...
            if (userDto != null)
            {
                ...
                var factoryIds = await base.Repo.Context.Queryable<Factory>()
                    .InnerJoin<UserFactory>((f, uf) => f.FacId == uf.FacId)
                    .Where((f, uf) => uf.UserId == userDto.UserId)
                    .Select((f, uf) => f.FacId)
                    .ToListAsync();
                userDto.FactoryIds = factoryIds;
            }
