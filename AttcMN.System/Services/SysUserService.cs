...
public class SysUserService : BaseService<SysUser, SysUserDto>, ITransient
{
    ...
    private readonly UserFactoryRepository _userFactoryRepository;
    ...
    public virtual int UpdateUser(SysUserDto user)
    {
        ...
        InsertUserPost(user);
        // 删除用户与工厂关联
        _userFactoryRepository.DeleteUserFactoryByUserId(user.UserId ?? 0);
        // 新增用户与工厂管理
        InsertUserFactory(user);
    ...
    /// <summary>
    /// Associates a user with one or more factories based on the provided user and factory identifiers.
    /// </summary>
    /// <remarks>The <paramref name="user"/> parameter must contain a valid user identifier. If the user
    /// identifier is null, it defaults to 0.</remarks>
    /// <param name="user">An object containing the user's identifier and the list of factory identifiers to associate with the user.</param>
    public void InsertUserFactory(SysUserDto user)
    {
        this.InsertUserFactory(user.UserId ?? 0, user.FactoryIds);
    }
    ...
    /// <summary>
    /// Associates a user with a collection of factory IDs by creating and inserting user-factory relationships.
    /// </summary>
    /// <remarks>This method creates a new user-factory relationship for each factory ID in the provided list
    /// and inserts them into the repository. If the <paramref name="facIds"/> list is empty, the method performs no
    /// operation.</remarks>
    /// <param name="userId">The unique identifier of the user to associate with the factories.</param>
    /// <param name="facIds">A list of factory IDs to associate with the user. The list must not be empty.</param>
    public void InsertUserFactory(long userId, List<int> facIds)
    {
        if (facIds.IsNotEmpty())
        {
            // 新增用户与角色管理
            List<UserFactory> list = new List<UserFactory>();
            foreach (int facID in facIds)
            {
                UserFactory ur = new UserFactory();
                ur.UserId = userId;
                ur.FacId = facID;
                list.Add(ur);
            }
            _userFactoryRepository.Insert(list);
        }
    }
    ...
    public virtual async Task<int> DeleteUserByIdsAsync(List<long> userIds)
    {
        ...
        _userFactoryRepository.DeleteUserFactory(userIds);
